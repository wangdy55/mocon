...
class MotionCtrl:
    def __init__(self, chara: Character, chara_ctrl):
        # self.test_bvh = BVHMotion("mocon/motion/mocap/part/walk_forward.bvh")
        # self.test_joint_trans, self.test_joint_orien = self.test_bvh.batch_fk()
        ...
        self.motions = []
        self.motions.append(BVHMotion("mocon/motion/mocap/part/walk_forward.bvh"))
        self.motions.append(BVHMotion("mocon/motion/mocap/part/idle.bvh"))
        self.motion_id = 0
    ...
    def _update_motion(
        ...
        if self.cur_state == "move":
            ...
            diff_root_pos[:, 1] = 0
            diff_root_rot = (
                R.from_quat(desired_rot_list[0:6]) * R.from_quat(cur_motion.joint_quat[key_frame, 0, :]).inv()
            ).as_rotvec()
